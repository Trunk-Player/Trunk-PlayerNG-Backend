    ...
    def handle(self, *args, **options):
        if CustomUser.objects.filter(email=options['UserEmail']):
            ...
            if User.userProfile.siteAdmin:
                self.stdout.write(self.style.WARNING(f"{options['UserEmail']} is Already a Site Admin"))
            else:
                ...
                self.stdout.write(self.style.SUCCESS(f"{options['UserEmail']} is now a Site Admin"))