    ...
    def handle(self, *args, **options):
        if CustomUser.objects.filter(email=options['UserEmail']):
            ...
            if User.userProfile.feedAllowed:
                self.stdout.write(self.style.WARNING(f"{options['UserEmail']} is Already a Feed Provider"))
            else:
                User.userProfile.feedAllowed = True
                User.userProfile.save()
                User.save()

                self.stdout.write(self.style.SUCCESS(f"{options['UserEmail']} is now a Feed Provider"))